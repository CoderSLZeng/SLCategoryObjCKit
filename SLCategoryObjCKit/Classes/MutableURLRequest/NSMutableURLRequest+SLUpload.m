//
//  NSMutableURLRequest+SLUpload.h
//  SLCategoryKit
//
//  Created by MutableURLRequest on 15/10/10.
//  Copyright (c) 2015年 MutableURLRequest. All rights reserved
//

#import "NSMutableURLRequest+SLUpload.h"

@implementation NSMutableURLRequest (SLUpload)

+ (instancetype)sl_requestWithURL:(NSURL *)URL fileURL:(NSURL *)fileURL name:(NSString *)name {
    return [self requestWithURL:URL fileURLs:@[fileURL] name:name];
}

+ (instancetype)sl_requestWithURL:(NSURL *)URL fileURL:(NSURL *)fileURL fileName:(NSString *)fileName name:(NSString *)name {
    return [self requestWithURL:URL fileURLs:@[fileURL] fileNames:@[fileName] name:name];
}

+ (instancetype)sl_requestWithURL:(NSURL *)URL fileURLs:(NSArray *)fileURLs name:(NSString *)name {
...
+ (instancetype)sl_requestWithURL:(NSURL *)URL fileURLs:(NSArray *)fileURLs fileNames:(NSArray *)fileNames name:(NSString *)name {
